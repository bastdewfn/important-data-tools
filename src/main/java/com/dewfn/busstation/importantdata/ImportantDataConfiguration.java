...
import com.dewfn.busstation.importantdata.filter.serializer.ImportantDataContextValueFilter;
...
public class ImportantDataConfiguration   {
    ...
    public ImportantDataMvcConfigurer importantDataMvcConfigurer(){
        return new ImportantDataMvcConfigurer();
    }

    public ImportantDataConfiguration(HttpMessageConverters httpMessageConverters){
        ...
        httpMessageConverters.forEach(httpMessageConverter -> {
            if(httpMessageConverter instanceof FastJsonHttpMessageConverter){
                ((FastJsonHttpMessageConverter) httpMessageConverter).getFastJsonConfig().setSerializeFilters(new ImportantDataContextValueFilter());
            }
            if(httpMessageConverter instanceof MappingJackson2HttpMessageConverter){
                ...
                FilterProvider filters = new SimpleFilterProvider().setFailOnUnknownId(false).addFilter("importantDataEncryptJacksonFilter", new ImportantDataEncryptJacksonFilter());
